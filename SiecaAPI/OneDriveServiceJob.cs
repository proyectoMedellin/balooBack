    ...
    private static async Task<IList<BeneficiaryAnthropometricDataEntity>> ToModelsAsync(SqlContext context, byte[] data)
    {
        ...
        try
        {
            ...
            while (await csv.ReadAsync())
            {
                ...
                {
                    Id = Guid.NewGuid(),
                    BeneficiaryId = await context.Beneficiaries.Where(x => x.DocumentNumber == csv.GetField("id")).Select(x => x.Id).FirstOrDefaultAsync(),
                    TrainingCenterId = await context.TrainingCenters.Where(x => x.Code == csv.GetField("doctor")).Select(x => x.Id).FirstOrDefaultAsync(),
